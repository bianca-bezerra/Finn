...
const DebitList = async () => {
    ...
    const renderTableBody = (data) => {
        ...
        data.forEach(debit => {
            ...
            valueCell.addEventListener('dblclick', (e) => editCell(e, debit, 'value'));
    ...
    const editCell = (e, debit, property) => {
        ...
        const handleEvents = async (e) => {
            if (e.key === 'Enter' || e.type === 'blur') {
                ...
                if (debit[property] != Number(input.value)) {
                    ...
                    await debits.put(debit.id, { category_id: Number(debit.category.id), value: debit.value, owner_id: debit.owner_id });
                }
            }
        };
